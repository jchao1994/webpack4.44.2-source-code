...
const applyWebpackOptionsDefaults = options => {
	...
	// mode模式，如development/production/none，默认为production
	// target部署目标，如 node/web/...，默认为web
	...
	// 定义一些默认值
	// options.devtool = development ? "eval" : false
	...
	// options.recordsInputPath = false
	D(options, "recordsInputPath", false);
	// options.recordsOutputPath = false
	D(options, "recordsOutputPath", false);
	// options.cache = development ? { type: "memory" } : false
	F(options, "cache", () =>
		development ? { type: /** @type {"memory"} */ ("memory") } : false
	);
	// 定义options.cache的默认属性
