...
class Compilation {
	 ...
	 * @property {ModuleFactory} factory // DllEntryDependency类构造器
	 * @property {Dependency[]} dependencies // [entry]
	 * @property {Module | null} originModule // null
	 ...
	 * @param {Dependency} dependency dependency used to create Module chain // entry，DllEntryDependency实例，带dependencies name type
	 ...
	 * @param {Dependency} entry entry dependency that should be followed // DllEntryDependency实例，带dependencies name type
	 * @param {string | EntryOptions} optionsOrName options or deprecated name of entry
	 * @param {ModuleCallback} callback callback function
	 * @returns {void} returns
	 */
	// 在lib/DllEntryPlugin.js调用了这个方法
	// entry => DllEntryDependency实例，带dependencies name type
	...
	_addEntryItem(context, entry, target, options, callback) {
		...
		// 有name且第一次，this.entries中name对应的entryData为undefined
		// 一个name对应唯一的entryData，以key-value的形式存放在this.entries中
		// 更新this.entries中存放的name对应的entryData
		if (entryData === undefined) {
			entryData = {
				dependencies: [], // 这个name对应的所有依赖entry(带dependencies name type)
				includeDependencies: [],
				options: {
					name: undefined,
					...options
			...
			// entry是DllEntryDependency实例，带dependencies name type
			// entryData.dependencies.push(entry)
			entryData[target].push(entry);
			// this.entris存放name-entryData
			this.entries.set(name, entryData);
		} else {
			// 更新entryData的dependencies和options
			entryData[target].push(entry);
			// 将options附加到entryData.options中，只能是添加，不能修改
			// 但是一旦出现冲突(值不同)，就报错
		...
		// this.hooks.addEntry.tap在lib/ProgressPlugin中
		// 添加entriesCount计数，每添加10个，就执行updateThrottled
		// 每过500ms，就执行update，节流delay为500ms
		...
		this.addModuleChain(context, entry, (err, module) => {
			...
			// 执行this.hooks.make.callAsync的回调
	...
	// 执行 this.hooks.finishModules.callAsync
			...
			if (chunkGraph.getNumberOfModuleChunks(module) > 0) {
				for (const runtime of chunkGraph.getModuleRuntimes(module)) {
					...
					const runtimeRequirements =
						this.codeGenerationResults.getRuntimeRequirements(module, runtime);
